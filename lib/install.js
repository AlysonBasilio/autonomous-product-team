...
const LEGACY_DIRS = [
  '.claude/skills/team-manager',
  '.claude/skills/team-member',
];

const MANIFEST = [
  { src: 'roles/team-lead.md', dest: '.claude/skills/team-lead/SKILL.md', transform: rewriteTaskPaths },
  { src: 'roles/teammate.md',  dest: '.claude/skills/teammate/SKILL.md' },
...
async function run({ force = false, dryRun = false } = {}) {
  ...
  for (const legacyDir of LEGACY_DIRS) {
    const legacyPath = path.join(projectRoot, legacyDir);
    if (fs.existsSync(legacyPath)) {
      if (!dryRun) fs.rmSync(legacyPath, { recursive: true, force: true });
      results.push({ dest: legacyDir, action: dryRun ? 'would remove (legacy)' : 'removed (legacy)' });
    }
  }
...
Next steps:
  ...
  "Use the team-lead agent to start working on my product"
...
function status() {
  ...
  if (fs.existsSync(settingsPath)) {
    try {
      ...
      settingsOk = hasAgentTeams && hasIdleHook;
